...
func (t Training) meanSpeed() float64 {
	return float64(t.distance()) / float64(t.Duration.Hours())
}
...
func (r Running) Calories() float64 {
	return ((18*r.meanSpeed() + 1.79) * r.Weight / MInKm * r.Duration.Hours() * MinInHours)
}
...
func (w Walking) Calories() float64 {
	return (0.035*w.Weight + ((math.Pow(w.meanSpeed()*1000/3600, 2)) / (w.Height / 100) * 0.029 * w.Weight)) * float64(w.Duration.Hours()) * MinInHours
}
...
// Формула расчета:
// (средняя_скорость_в_км/ч + SwimmingCaloriesMeanSpeedShift)*
// SwimmingCaloriesWeightMultiplier * вес_спортсмена_в_кг * время_тренировки_в_часах
// Это переопределенный метод Calories() из Training.
func (s Swimming) Calories() float64 {
	return (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * float64(s.Duration.Hours()) * s.Weight
}
